...
const ProductsMarca = () => {
  ...
  // Filtrado con animación por marca
  const filterItems = (marca) => {
    const filtered =
      marca === "all"
        ? products
        : products.filter((item) => item.marca === marca);
  ...
  // aplicar filtro si viene marca en la URL
  useEffect(() => {
    if (products.length > 0) {
      if (marcaFromUrl && marcaFromUrl !== "all") {
        filterItems(marcaFromUrl);
      } else {
        showAll();
      }
    }
  }, [marcaFromUrl, products]);
  ...
  const marcas = React.useMemo(() => {
    ...
    return [
      ...
      ...uniqueMarcas.map(marca => ({ name: marca, value: marca }))
    ];
  }, [products]);
  ...
  return (
    ...
    >
            ...
            {showFilters && (
              ...
              >
                {marcas.map((marca, index) => (
                  <motion.button
                    key={marca.value}
                    onClick={() => marca.value === "all" ? showAll() : filterItems(marca.value)}
                    className={`text-sm py-1 px-2 transition-all duration-200 ${
                      selectedMarca === marca.value
      ...
      {/* Cards con animación */}
      <motion.div
        key={selectedMarca + sortOption} // Fuerza re-render con animación
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
